...
export default function MapPage() {
  ...
  const handleNewEvent = async (event: FoodEvent) => {
    ...
    try {
      console.log('Attempting to add new event with data:', { 
  ...
  // Sort upcoming events: live events first, then others
  ...
  const applyFilters = (events: Issue[]) => {
    return events.filter(event => {
      ...
      // Time frame filter (hour only, 12-hour to 24-hour)
      if (filterStartHour) {
  ...
  // Pin click handler: bring event to top and expand, or toggle off if already selected
  const handlePinClick = (eventId: string) => {
    if (selectedPinId === eventId) {
      ...
      // Scroll the event into view in the sidebar
  ...
  // When editing, always select the editing event's pin
  useEffect(() => {
  ...
  // Handle pin drag end (for editing)
  const handlePinDragEnd = async (eventId: string, lngLat: { lng: number; lat: number }) => {
    ...
    try {
      ...
      // Also update editingEvent state
  ...
  return (
    <main className={styles.main}>
      ...
      <div style={{ width: '100vw', height: '90vh', position: 'relative' }}>
        ...
        {/* Report Form overlayed on the right */}
        {selectedLocation && (
